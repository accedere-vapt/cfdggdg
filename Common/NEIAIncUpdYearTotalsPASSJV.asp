...
if lstrDrCrFlg = "CR" then
	if lstrFiscalYearForTxnDt <> lstrFiscalYearForProcessing then
		mstrSQL = mstrSQL & "open_cr_bal =  NVL(open_cr_bal,0) + " & ldblTxnAmt & ", "
	end if
	mstrSQL = mstrSQL & "last_cr_bal = NVL(last_cr_bal,0) + " & ldblTxnAmt & ", "
else
	if lstrFiscalYearForTxnDt <> lstrFiscalYearForProcessing then
		mstrSQL = mstrSQL & "open_dr_bal = NVL( open_dr_bal,0) + " & ldblTxnAmt & ", "
	end if
	mstrSQL = mstrSQL & "last_dr_bal =  NVL(last_dr_bal,0) + " & ldblTxnAmt & ", "				
end if
...
If lintRecords = 0 and Err.number = 0 then
	...
	if lstrFiscalYearForTxnDt <> lstrFiscalYearForProcessing then
		if lstrDrCrFlg = "CR" then
			ldblOpenCrBal = ldblTxnAmt
		else
			ldblOpenDrBal = ldblTxnAmt
		end if
	end if
