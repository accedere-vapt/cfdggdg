...
<%
'**********************************************************
' This File Contains all common NEIA Functions
'**********************************************************
...
Function fnintIsCalendarOpen(astrLogicalLocCd, astrTxnType, astrTxnDt )
	...
	'Response.Write astrTxnDt '& " "  & ldtmTmpDt & " " & lstrTxnMonth & " " & lstrTxnYear
	mstrSQL = "select * "
...
function fnintPassGLTxn(aconEcgcDb, aarrTxnHdr,aarrTxnDtl)
	...
	'Response.Write "<>"&lstrLogicalLocCd&"<>"&lstrTxnType&"<>"&lstrTxnDt&"<>"
	'Response.end
	'Response.Write "<BR>"&lstrLogicalLocCd&"<BR>"&lstrTxnType&"<BR>"&lstrTxnDt&"<BR>"

	'Commenting this as third parameter should be transaction date, Kaustubh, 23-02-2005
	'lintCalendarOpen = fnintIsCalendarOpen(lstrLogicalLocCd,lstrTxnType,lstrTxnType)
	...
	'Response.write lstrFiscalYearForTxnDt
	...
	if Err.number <> 0 then
		'Response.Write "<FONT class=clsError>fnintPassGLTxn: lrsFiscalYr Open</FONT><BR>"
	...
	'Response.Write  lrsFiscalYr.Fields("last_yr_closed") &":" & lstrFiscalYearForTxnDt
	'response.end
	...
	if Err.number <> 0 then
		'Response.Write "<FONT class=clsError>fnintPassGLTxn: lrsGLTxn Open</FONT><BR>"
		'Response.End
		'Response.Redirect "../../../Common/Error.asp?aintCode=4&aintPage=-1&astrErrNumber="&Err.Number&"&astrErrDescription="&Err.description
		fnintPassGLTxn = -4
		exit function
	...
	If Err.number <> 0 then
		'Response.Write "<FONT class=clsError>fnintPassGLTxn: TXN Header Insert</FONT><BR>"
		'Response.End
		'Response.Redirect "../../../Common/Error.asp?aintCode=4&aintPage=-1&astrErrNumber="&Err.Number&"&astrErrDescription="&Err.description
		fnintPassGLTxn = -4
		exit function
	...
	for i = 0 to ubound(aarrTxnDtl,2)
		...
		if Err.number <> 0 then
			'Response.Write "<FONT class=clsError>fnintPassGLTxn: Entity GL Master Select</FONT><BR>"
			'Response.End
			'Response.Redirect "../../../Common/Error.asp?aintCode=4&aintPage=-1&astrErrNumber="&Err.Number&"&astrErrDescription="&Err.description
			fnintPassGLTxn = -4
			exit function
		...
		'Response.Write lstrPersonalLedgerLevel & " : " & lstrPersonalLedgerCd & ":" & lstrLogicalLocCd
		'response.End
...
'Response.End
			Select Case ucase(lstrPersonalLedgerLevel)
				...
				'the following code is modified by vikas wipro 7-12-2005

				Case "CONTRACTPARTY+INVESTMENTNUMBER"

					arrTemp = fnGetContractorDtls(lstrPersonalLedgerCd,lstrLogicalLocCd)
				case else
					'Response.Write "Cannot proceed, new Personal Ledger Level encountered"
		...
		aconEcgcDb.Execute mstrSQL
'Response.Write mstrSQL & "<BR>" & Err.number
	'response.end
		If Err.number <> 0 then
		...
		<!-- #include file="NEIAIncUpdMonthTotals.asp"-->
		<!-- #include file="NEIAIncUpdYearTotals.asp"-->
		...
		'Response.Write "error .." & err.number & " " & err.description  & i
		...
		if ucase(lstrDrCrFlg) = "DR" then
			ldblGLBrBal = ldblGLBrBal - ldblTxnAmt
		else
			ldblGLBrBal = ldblGLBrBal + ldblTxnAmt
		end if
...
' below if condition chnaged by Suhas D as per Mukesh and vipul Sir Suggested on 07/02/2013 -- as we are not ussing this error detilas table
            If  Err.number <> 0 then
   ...
   ' below if condition chnaged by Suhas D as per Mukesh and vipul Sir Suggested on 07/02/2013 -- as we are not ussing this error detilas table
                 If  Err.number <> 0 then
				'If lintRecords <> 1 or Err.number <> 0 then
					Response.Write "<FONT class=clsError>fnintPassGLTxn: Error Close</FONT><BR>" &i
					Response.End
					'Response.Redirect "../../../Common/Error.asp?aintCode=4&aintPage=-1&astrErrNumber="&Err.Number&"&astrErrDescription="&Err.description
					fnintPassGLTxn = -4
					exit function
...
function fnintReverseGLTxn(aconEcgcDb, astrLogicalLocCd, astrTxnType, alngTxnNo, astrReversalDt, astrReversalReason, astrUserID)
	...
	'MUKESH KAPOOR MODIFIED THE FUNCTION NAME FROM "fnintNEIAPassGLTxn" TO "fnintPassGLTxn"
...
Function fngetGLBranchBalance(aconEcgcDb, astrLogicalLocCd, aintMainGLCd, aintSubGLCd1, aintSubGLCd2, aintSubGLCd3, aintSubGLCd4, astrPersonalLedgerCd, astrTxnDt)
	...
	With lobjCmd
		...
		Set param7 = .CreateParameter("ipPersonal_Ledger_Cd", adVarChar, adParamInput, 150, astrPersonalLedgerCd)
...
Function fnGetExporterDtls(astrIECd)
	...
	strSQL = strSQL & "where UPPER(ie_cd) = UPPER('" & astrIECd & "')"
...
Function fnGetEmployeeDtls(aintEmpNo)
	...
	if not rsCommon.EOF then
		...
		'Response.Write "Error Desc. " & Err.Description & " "
		'Response.End
		Response.Redirect "../../../Common/Error.asp?aintCode=2&aintPage=-1&astrErrNumber="&Err.Number&"&astrErrDescription=Employee record does not exist.EMP NO :" & intEmpNo
...
Function fnGetPolicyDtls(astrPolNo)
	...
	strSQL = strSQL & "where UPPER(policy_no) = UPPER('" & astrPolNo & "')"
...
Function fnGetStatusAgencyDtls(astrAgencyCd)
	...
	strSQL = strSQL & "where UPPER(AGENCY_CD) = UPPER('" & astrAgencyCd & "')"
...
'Date          : 03-12-2001
'**********************************************************
Function fnGetGLDetails(astrMainGlCd, astrSubGlCd1, astrSubGlCd2, astrSubGlCd3, astrSubGlCd4)
...
'**********************************************************
'Function name : fnGetGLDetails for Budget 4th level error
'Purpose       : This will get General Ledger Details
'Input         : GL Code fields
'Output        :
'Author        : Amit C
'Date          : 03-12-2001
'**********************************************************
Function fnGetGLDetailBudget(astrMainGlCd, astrSubGlCd1, astrSubGlCd2, astrSubGlCd3, astrSubGlCd4)
...
'**********************************************************
'Function name : fngetCurrFiscalYearEndDt
'Purpose       : Retrieve the Current Fiscal Year End Date
'Input         :
'Output        : return Current Fiscal Year End Date

'Author        : Kaustubh Deshpande
'Date          : 05-05-2005
'**********************************************************
Function fngetCurrFiscalYearEndDt()
	...
	lstrSQL = "select to_char(add_months(curr_fisc_yr_start_dt,12)-1,'DD/MM/YYYY') "
	lstrSQL = lstrSQL & "from NEIA_FISCAL_YR "
	lstrSQL = lstrSQL & "where entity_cd = 'NEIA' "
	'Response.Write lstrSQL
	'Response.End
	...
	if not lrsFiscalYear.EOF then
		fngetCurrFiscalYearEndDt  = lrsFiscalYear.Fields(0)
