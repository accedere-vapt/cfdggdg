...
if request.form("btnSubmit") = "Revoke" then
	...
	SQL = " update ECGC.adm_lease_rent_revoke_request set status='A',MODIFIED_BY= '"&Session("sEmployeeNo")&"',MODIFIED_DT=sysdate where emp_no='"&IntEmpNo&"' and txn_no='"&TxnNo&"' and status='P' "
	Con.Execute SQL
...
<%
 ...
 SQL = " SELECT * FROM ADM_LEASE_PYMT_SCHEDULE where emp_no='"&IntEmpNo&"' and txn_no='"&TxnNo&"' and status='Paid' order by LEASE_TO_DATE"
 Rs.Open SQL,Con
...
<%
 Set Rs2 = server.CreateObject("ADODB.Recordset")
 SQL = " SELECT * FROM ADM_LEASE_PYMT_SCHEDULE where emp_no='"&IntEmpNo&"' and txn_no='"&TxnNo&"' and status='NotPaid' order by LEASE_TO_DATE"
 Rs2.Open SQL,Con
 RevokedDt  = Rs2(3)
%> 	
<%
